...
## 📂 Project Structure
* `backend/controllers/`: Contains the business logic for handling project data and file paths.
* `backend/models/`: Defines the Mongoose Schema for the Project entity.
* `backend/uploads/`: Dedicated directory for storing uploaded assets, automatically generated by `server.js`.
* `frontend/src/components/`: Modular React components like `TaskForm` for project submission.
